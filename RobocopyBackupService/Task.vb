...
Public Class Task
    ...
    Public Sub Start()
        ...
        Dim newThread As New Thread(Sub() SysUtils.RunBackup(Me)) ' Crea un nuovo thread
        newThread.Start() ' Avvia il thread

        threadList.Add((newThread, cts)) ' Aggiungi il Thread e il CTS alla lista
    End Sub
    Private threadList As New List(Of (Thread As Thread, CTS As CancellationTokenSource))

    Public Sub [Stop]()
        ' Controlla che la lista non sia vuota
        If threadList IsNot Nothing AndAlso threadList.Count > 0 Then
            For Each t As (Thread As Thread, CTS As CancellationTokenSource) In threadList
                t.CTS.Cancel() ' Cancella ogni Thread associato
                t.Thread.Join() ' Aspetta che il thread finisca (sincrono)
            Next
            threadList.Clear() ' Svuota la lista
        Else
            'Console.WriteLine("La lista dei thread è vuota o nulla.")
        End If
    End Sub
